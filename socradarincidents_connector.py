...
from socradarincidents_consts import (
    SOCRADARINCIDENTS_CONNECTIVITY_ENDPOINT,
    SOCRADARINCIDENTS_ERR_CONNECTIVITY_TEST,
    SOCRADARINCIDENTS_SUCC_CONNECTIVITY_TEST,
)


class SocradarincidentsConnector(BaseConnector):
    """
    Socradarincidents connector class that serves as a starting point for new connectors.
    """
    ...
    def _handle_test_connectivity(self, param):
        ...
        endpoint = SOCRADARINCIDENTS_CONNECTIVITY_ENDPOINT
        headers = {"Authorization": f"Bearer {self._api_key}"}

        ret_val, _ = self._make_rest_call(endpoint, action_result, headers=headers)

        if phantom.is_fail(ret_val):
            self.save_progress(SOCRADARINCIDENTS_ERR_CONNECTIVITY_TEST)
            return action_result.get_status()

        self.save_progress(SOCRADARINCIDENTS_SUCC_CONNECTIVITY_TEST)
